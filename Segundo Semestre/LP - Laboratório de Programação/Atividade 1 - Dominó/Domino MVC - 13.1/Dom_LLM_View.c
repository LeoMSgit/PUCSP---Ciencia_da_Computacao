...
void jogarPeca() {
    ...
    // Remove a peça da mão do jogador usando a nova função
    removerPeca(&jogadores[0], indicePeca);
...
void comprarPeca(Jogador *jogador) {
    if (monte.quantidadeRestante > 0) {
        jogador->pecas[jogador->quantidadePecas++] = monte.pecasRestantes[monte.quantidadeRestante - 1];
        
        monte.quantidadeRestante--;
        printf("Você comprou a peça [%d|%d]\n", jogador->pecas[jogador->quantidadePecas - 1].lado1, jogador->pecas[jogador->quantidadePecas - 1].lado2);
    } else {
        printf("Não há mais peças disponíveis no monte.\n");
    }
}
