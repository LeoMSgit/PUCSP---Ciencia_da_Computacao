...
void distribuirPecas(Jogador jogadores[], PecaDomino pecasEmbaralhadas[], int numJogadores) {
    int indicePeca = 0;  // Índice para as peças embaralhadas

    // Distribui as peças entre os jogadores
    for (int i = 0; i < numJogadores; i++) {
        jogadores[i].quantidadePecas = 0; // Inicializa a quantidade de peças do jogador
        jogadores[i].contadorCompras = 0; // Inicializa o contador de compras
        jogadores[i].jogadasRealizadas = 0; // Inicializa o contador de jogadas

        // Cada jogador recebe PECAS_POR_JOGADOR / 2 peças
        for (int j = 0; j < PECAS_POR_JOGADOR / 2; j++) {
            if (indicePeca < TOTAL_PECAS) { // Verifica se ainda há peças disponíveis
