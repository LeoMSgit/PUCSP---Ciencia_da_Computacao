...
void exibirMenuAcoes(Jogador *jogador) {
    int opcao;

    do {
        printf("\n\nEscolha uma acao:\n");

        // Se o jogador já jogou, muda a opção de Jogar para Passar
        if (jogador->jaJogou) {
            printf("1. Passar a vez\n");
        } else {
            printf("1. Jogar peca\n");
        }
        ...
        switch (opcao) {
            case 1:
                if (!jogador->jaJogou) {
                    jogarPeca(jogador); // Jogador joga uma peça
                    jogador->jaJogou = true; // Atualiza o estado para que o jogador tenha jogado
                } else {
                    printf("Voce ja jogou nesta rodada. Passe a vez.\n");
                }
                break;
            case 2:
                comprarPeca(jogador);
	       ...
	       case 4:
			{
			    ...
			    // Inicia o menu de ações para o jogador inicial
			    exibirMenuAcoes(&jogadores[jogadorInicial]); // Chama a função com o jogador correto
			    break;
			}
            case 0:
                printf("Saindo...\n\n");
