...
void embaralharPecas(PecaDomino pecas[], int tamanho) {
    for (int i = tamanho - 1; i > 0; i--) {
        ...
        int sorteio = rand() % (i + 1); // (i + 1) para incluir o i no intervalo
        // Troca as peças
...
void exibirPecasJogadores(Jogador jogadores[], int numeroJogadores) {
    for (int i = 0; i < numeroJogadores; i++) {
        ...
        for (int j = 0; j < jogadores[i].quantidadePecas; j++) {
            printf("[%d|%d] ", jogadores[i].pecas[j].lado1, jogadores[i].pecas[j].lado2);
        }
        printf("\n");
    }
}
...
void inicializarMonte(PecaDomino pecas[], int quantidadeDistribuida) {
    ...
    // Preenche o monte com as peças restantes após a distribuição
    for (int i = quantidadeDistribuida; i < TOTAL_PECAS; i++) {
        monte.pecasRestantes[indiceMonte] = pecas[i];
        indiceMonte++;
    }

    monte.quantidadeRestante = TOTAL_PECAS - quantidadeDistribuida; // Calcula corretamente as peças restantes
    embaralharPecas(monte.pecasRestantes, monte.quantidadeRestante); // Embaralha as peças restantes
}

// Função para distribuir as peças para os jogadores
void distribuirPecas(Jogador jogadores[], PecaDomino pecas[], int numeroJogadores) {
    int pecasPorJogador = 7; // Cada jogador recebe 7 peças

    for (int i = 0; i < numeroJogadores; i++) {
        jogadores[i].quantidadePecas = pecasPorJogador; // Define a quantidade de peças para o jogador
        for (int j = 0; j < pecasPorJogador; j++) {
            jogadores[i].pecas[j] = pecas[i * pecasPorJogador + j]; // Distribui as peças
        }
    }
}
