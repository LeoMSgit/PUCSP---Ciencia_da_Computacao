...
void jogarPeca(Jogador jogadores[], int jogadorAtual) {
    int escolha;
    int indicePeca;
    int ladoEscolhido;
    int jogadaValidaMarcador;
    PecaDomino pecaEscolhida;
    ...
    if (jogadores[jogadorAtual].quantidadePecas == 0) {
    ...
    // Loop para a jogada
    do {
        // Exibe as peças na mão do jogador atual
        printf("Escolha uma peca para jogar:\n");
        for (int i = 0; i < jogadores[jogadorAtual].quantidadePecas; i++) {
            printf("%d. [%d|%d]\n", i + 1, jogadores[jogadorAtual].pecas[i].lado1, jogadores[jogadorAtual].pecas[i].lado2);
        }
        ...
        if (escolha < 1 || escolha > jogadores[jogadorAtual].quantidadePecas) {
            printf("Escolha invalida. Tente novamente.\n");
            continue; // Pede nova entrada
        }

        indicePeca = escolha - 1; // Ajusta o índice para a peça escolhida
        pecaEscolhida = jogadores[jogadorAtual].pecas[indicePeca]; // Atribui a peça escolhida
        ...
        // Remove a peça da mão do jogador atual
        for (int i = indicePeca; i < jogadores[jogadorAtual].quantidadePecas - 1; i++) {
            jogadores[jogadorAtual].pecas[i] = jogadores[jogadorAtual].pecas[i + 1];
        }
        jogadores[jogadorAtual].quantidadePecas--;

        // Adiciona a peça à mesa
        adicionarPecaMesa(pecaEscolhida, ladoEscolhido);

        // Incrementa o contador de jogadas realizadas do jogador atual
        jogadores[jogadorAtual].jogadasRealizadas++;
...
void exibirMenuAcoes(int jogadorAtual) {
    ...
    do {
    	printf("Pecas restantes no monte antes da compra: %d\n", monte.quantidadeRestante);
        ...
        switch (opcao) {
             case 1:
		        if (jogadores[jogadorAtual].jogadasRealizadas == 0) {
		            jogarPeca(jogadores, jogadorAtual); // Passando jogadores e jogadorAtual
		        } else {
            ...
            case 3:
    			imprimirPecasMao(jogadores[jogadorAtual]); // Exibe a mão do jogador atual
                break;
            case 4:
...
void exibirMenuInicial() {
    ...
    while (1) { // Loop infinito até o BREAK
        ...
        printf("4 - Carregar Jogo (Indisponivel na Versao Atual)\n");
        printf("Digite sua opcao: ");
        ...
        if (resultado != 1) {
            limparBuffer();
            printf("\nOpcao invalida!\n\n");
        } else if (numeroJogadores < 1 || numeroJogadores > 4) {
            printf("\nOpcao invalida! Digite 1, 2 ou 3.\n\n");
            limparBuffer();
        } else {
            // Se a opção for 1 ou 4
            if (numeroJogadores == 1 || numeroJogadores == 4) {
            ...
            case 4:
				{
				    ...
				    inicializarMonte(tipo.embaralhadas, numeroJogadores * 7);
