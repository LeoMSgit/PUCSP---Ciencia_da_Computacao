...
void jogarPeca() {
    ...
    } while (!jogadaValidaMarcador); // Continua enquanto a jogada não for válida
}
...
void exibirMenuAcoes(int jogadorAtual) { // Adicione jogadorAtual como parâmetro
    int opcao;
        ...
        switch (opcao) {
            case 1:
                if (jogadasRestantes > 0) {
                    ...
                    printf("Você ja realizou sua jogada.\n");
                }
                break;
            case 2: {
            ...
            case 3:
                imprimirPecasMao(jogadores[0]); // Exibe a mão do Jogador [N]
                break;
            case 4:
